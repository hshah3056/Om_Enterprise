...
class TestHelpdeskTicketBase(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_own = new_test_user(
            ...
            groups="helpdesk_custom.group_helpdesk_user_own",
            context=ctx,
        )
        cls.user_team = new_test_user(
            cls.env,
            login="helpdesk_mgmt-user_team",
            groups="helpdesk_custom.group_helpdesk_user_team",
            context=ctx,
        )
        cls.user = new_test_user(
            cls.env,
            login="helpdesk_mgmt-user",
            groups="helpdesk_custom.group_helpdesk_user",
            context=ctx,
        )
        cls.stage_closed = cls.env.ref("helpdesk_custom.helpdesk_ticket_stage_done")
        ...
        cls.new_stage = cls.env.ref("helpdesk_custom.helpdesk_ticket_stage_new")
